...
<#
.SYNOPSIS
    Builds and launches the Sword of the Samurai age trainer.

.DESCRIPTION
    Restores/builds the WPF project with `dotnet build`, then starts the produced
    SotSAgeTrainer.exe. The trainer freezes the protagonist's life-stage at "Youth"
    by writing to the live DOSBox process. The app's manifest requests administrator
    rights (needed to read/write the game process's memory), so Windows shows a UAC
    prompt on launch.

    Once the window is open: load your game in DOSBox (Restore/Continue), then click
    "Freeze age" with Target = Youth.
...
.EXAMPLE
    .\Run.ps1 -NoBuild
    Launches the already-built trainer without touching the build.
#>
[CmdletBinding()]
param(
...
$testProject = Join-Path $root 'SwordSamuraiTrainer\tools\Verify\Verify.csproj'
$exeName     = 'SotSAgeTrainer.exe'
$exePath     = $null   # lands in a RID subfolder; resolved by recursive search
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Load your game in DOSBox (Restore/Continue), then click "Freeze age" with Target = Youth.'
# ------------------------------------------------------------------------------
