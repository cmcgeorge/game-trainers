...
<#
.SYNOPSIS
    Builds and launches the Space 1889 trainer.
...
.DESCRIPTION
    ...
    read/write DOSBox's memory), so Windows shows a UAC prompt on launch.
...
.EXAMPLE
    ...
    Builds Debug, runs the format checks (415 when a Space 1889 folder is found; set
    S1889_GAME_DIR to point at one), and does not launch the GUI.

.EXAMPLE
    dotnet run --project test\FormatCheck -- "C:\GAMES\S1889" --live
    Runs the checks against that game folder and additionally attaches (read-only)
    to a running DOSBox to verify the locator end to end. Skipped rather than
    failed when no emulator is running.
#>
[CmdletBinding()]
param(
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\Space1889Trainer\Space1889Trainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'Space1889Trainer.exe'
$exePath     = Join-Path $root "src\Space1889Trainer\bin\$Configuration\net8.0-windows\Space1889Trainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Start 1889 in DOSBox, begin or load a game, then press Attach in the trainer.'
# ------------------------------------------------------------------------------
...
Write-Host "Tip: game time runs in real time - use GAME > PAUSE in the game while you edit." -ForegroundColor DarkGray
