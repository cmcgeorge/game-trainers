<#
.SYNOPSIS
    Builds and launches the Eye of the Beholder I trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\EyeOfTheBeholder1Trainer\EyeOfTheBeholder1Trainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'EOB1Trainer.exe'
$exePath     = Join-Path $root "src\EyeOfTheBeholder1Trainer\bin\$Configuration\net8.0-windows\EOB1Trainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Eye of the Beholder in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
