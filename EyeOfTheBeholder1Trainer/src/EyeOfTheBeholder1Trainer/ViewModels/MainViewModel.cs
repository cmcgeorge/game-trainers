...
/// and the party-wide quick actions.
/// </summary>
public sealed class MainViewModel : ObservableObject, ICharacterHost, IDisposable
{
    ...
    private GameTrainers.Common.Memory.ProcessMemory? _mem;
    ...
    private string _status = "Launch Eye of the Beholder in DOSBox, then pick the process and Attach.";
    ...
    {
        get => _freezeFood;
        set { if (SetField(ref _freezeFood, value)) { foreach (var c in Party) c.FreezeFood = value; Status = value ? "Food frozen for the party." : "Food freeze OFF."; } }
    }
    ...
    public ICommand MaxHpPartyCommand { get; }
    ...
    public MainViewModel()
    {
        ...
        MaxHpPartyCommand = new RelayCommand(_ => ForEachParty(c => c.MaxHp()), _ => Party.Count > 0);
    ...
    private void Attach()
    {
        ...
        try
        {
            _mem = GameTrainers.Common.Memory.ProcessMemory.Open(SelectedProcess.Id);
    ...
    private void PollTick()
    {
        ...
        {
            if (_mem.Read(c.Address, _pollBuf, CharacterFormat.RecordSize) == CharacterFormat.RecordSize)
    ...
    private void RaiseCommands()
    {
        ...
        (MaxHpPartyCommand as RelayCommand)?.RaiseCanExecuteChanged();
