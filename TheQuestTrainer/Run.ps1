<#
.SYNOPSIS
    Builds and launches The Quest trainer.
...
.DESCRIPTION
    ...
    The Quest is a native 32-bit Windows game, so there is no DOSBox in the way: start
    TheQuest.exe, load or begin a game, then press Attach. The trainer finds the character
    record by itself — first from the module's own engine pointer, and failing that by sweeping
...
$root         = $PSScriptRoot
$project      = Join-Path $root 'src\TheQuestTrainer\TheQuestTrainer.csproj'
$testProject  = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName      = 'TheQuestTrainer.exe'
$exePath      = Join-Path $root "src\TheQuestTrainer\bin\$Configuration\net8.0-windows\TheQuestTrainer.exe"
$binConfigDir = 'bin'
$buildArgs    = @()
$launchHint   = 'Start TheQuest.exe and load or begin a game, then Attach. The trainer locates the character record in a few milliseconds - no value searching.'
# ------------------------------------------------------------------------------
