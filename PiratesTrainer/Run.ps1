...
<#
.SYNOPSIS
    Builds and launches the Sid Meier's Pirates! trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\PiratesTrainer\PiratesTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'PiratesTrainer.exe'
$exePath     = Join-Path $root "src\PiratesTrainer\bin\$Configuration\net8.0-windows\PiratesTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Start Pirates! under DOSBox (run PIR.EXE) and get into a game, then in the Live tab pick the dosbox process, Attach, and click "Auto-locate".'
# ------------------------------------------------------------------------------
