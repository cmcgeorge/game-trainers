<#
.SYNOPSIS
    Builds and launches the Knights of Legend trainer.
...
.PARAMETER Test
    Run the verification harness after building.
...
.EXAMPLE
    ...
    Builds Debug and does not launch the GUI.
#>
[CmdletBinding()]
param(
...
if ($Clean -and $NoBuild -and -not $NoRun) {
    throw '-Clean and -NoBuild together would delete the executable and then try to launch it. Add -NoRun or drop -Clean.'
}

$root        = $PSScriptRoot
$project     = Join-Path $root 'src\KnightsOfLegendTrainer\KnightsOfLegendTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'KnightsOfLegendTrainer.exe'
$exePath     = Join-Path $root "src\KnightsOfLegendTrainer\bin\$Configuration\net8.0-windows\KnightsOfLegendTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Knights of Legend in DOSBox, then Attach in the trainer.'
...
if (-not (Test-Path -LiteralPath $project)) {
    throw "Project not found at '$project'. Run this script from the KnightsOfLegendTrainer folder."
}
...
if ($Clean) {
    ...
    $cleanDirs = @([System.IO.Path]::GetDirectoryName($project), (Split-Path -Parent $testProject))
...
if ($Test) {
    Write-Step "Building test harness ($Configuration)"
    dotnet build $testProject -c $Configuration @buildArgs -v minimal
    if ($LASTEXITCODE -ne 0) { throw "Test build failed (exit code $LASTEXITCODE)." }
    Write-Step 'Running FormatCheck'
    dotnet run --project $testProject -c $Configuration --no-build
    if ($LASTEXITCODE -ne 0) { throw "FormatCheck failed (exit code $LASTEXITCODE)." }
    if ($NoRun) { return }
}
