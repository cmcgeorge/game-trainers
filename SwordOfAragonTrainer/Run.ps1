<#
.SYNOPSIS
    Builds and launches the Sword of Aragon trainer.
...
.DESCRIPTION
    ...
    read/write the DOSBox process's memory for the Live tab), so Windows shows a
    UAC prompt on launch. Save-file editing does not need admin rights, but the
    manifest applies to the whole app.
...
$root         = $PSScriptRoot
$project      = Join-Path $root 'src\SwordOfAragonTrainer\SwordOfAragonTrainer.csproj'
$testProject  = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName      = 'SwordOfAragonTrainer.exe'
$exePath      = Join-Path $root "src\SwordOfAragonTrainer\bin\$Configuration\net8.0-windows\SwordOfAragonTrainer.exe"
$binConfigDir = 'bin'
$buildArgs    = @()
$launchHint   = 'Open an ARAGON.HS? save to edit it, or start DOSBox and Attach for live editing.'
# ------------------------------------------------------------------------------
