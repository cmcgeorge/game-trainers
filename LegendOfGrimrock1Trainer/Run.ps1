<#
.SYNOPSIS
    Builds and launches the Legend of Grimrock trainer.

.DESCRIPTION
    Restores/builds the WPF project with `dotnet build`, then starts the produced executable.
    The app's manifest requests administrator rights (needed to read/write the game process's
    memory), so Windows shows a UAC prompt on launch.

    Legend of Grimrock is a native 32-bit Windows game, so there is no DOSBox in the way: start
    grimrock.exe, load or begin a game, then press Attach. The trainer finds the game's LuaJIT
    virtual machine by itself and reads the party straight out of it — there is no value scanning
    at any point.
...
.PARAMETER Test
    Run the verification harness after building. It needs no game and no copyrighted files.
...
$root         = $PSScriptRoot
$project      = Join-Path $root 'src\LegendOfGrimrock1Trainer\LegendOfGrimrock1Trainer.csproj'
$testProject  = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName      = 'GrimrockTrainer.exe'
$exePath      = Join-Path $root "src\LegendOfGrimrock1Trainer\bin\$Configuration\net8.0-windows\GrimrockTrainer.exe"
$binConfigDir = 'bin'
$buildArgs    = @()
$launchHint   = 'Start grimrock.exe and load or begin a game, then Attach. The trainer locates the game''s Lua VM in a few milliseconds - no value searching.'
# ------------------------------------------------------------------------------
...
if (-not (Test-Path -LiteralPath $project)) {
    throw "Project not found at '$project'. Run this script from the trainer folder."
}
