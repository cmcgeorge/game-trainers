...
/// Grimrock sets DYNAMICBASE, so the preferred image base in the header is <i>not</i> where the
/// module is, and the static <c>lua_State</c> slot is only trusted when its RVA actually lands in a
/// writable data section of the image that is really mapped.
/// </summary>
public sealed class PeImage
{
    ...
    /// <summary>Link timestamp — the cheapest way to tell two Grimrock builds apart.</summary>
    public uint TimeDateStamp { get; private init; }

    /// <summary>Preferred load address from the optional header. Grimrock is relocated away from it.</summary>
    ...
    /// <summary>Whether the image opted in to ASLR. Grimrock does, which is why nothing is hard-coded.</summary>
    public bool HasAslr => (DllCharacteristics & 0x0040) != 0;

    /// <summary>Whether this is the 32-bit x86 image the Lua layout assumes.</summary>
    ...
    public static PeImage? Parse(ReadOnlySpan<byte> header)
    {
        ...
        int opt = coff + 20;
        if (opt + 2 > header.Length) return null;
        if (BitConverter.ToUInt16(header.Slice(opt, 2)) != Pe32Magic) return null;   // PE32 only
        if (opt + 72 > header.Length) return null;      // DllCharacteristics is a ushort at +70
