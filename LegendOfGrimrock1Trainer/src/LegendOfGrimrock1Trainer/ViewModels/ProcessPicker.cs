...
/// trainer's own executable is <c>GrimrockTrainer.exe</c>, whose process name contains "grimrock"
/// and sorts <i>before</i> <c>grimrock</c> under an ordinal comparison. A picker that merely
/// substring-matched and then sorted by name would auto-select the trainer itself, attach to a
/// 64-bit .NET process, and then report that no LuaJIT VM could be found. Exact matches therefore
