...
public sealed class MonsterEntryViewModel
{
    ...
    public string Plural => Monster.Plural;
    public string Group => Monster.Group;
    public string IdTag => Monster.IdTag;
    public string Description => Monster.DetailText;
}

/// <summary>
/// Read-only reference for the Monsters tab: the game's complete 127-entry bestiary grouped
...
public sealed class MonsterReferenceViewModel : ObservableObject
{
    ...
    /// <summary>Live filter text; matches a monster's name, plural form (substring) or id.</summary>
    public string SearchText
    {
        get => _search;
        set { if (SetField(ref _search, value ?? "")) { _query = _search.Trim(); Monsters.Refresh(); } }
    }
    ...
    private bool Matches(object o)
    {
        ...
        return m.Name.Contains(_query, StringComparison.OrdinalIgnoreCase)
            || m.Plural.Contains(_query, StringComparison.OrdinalIgnoreCase)
