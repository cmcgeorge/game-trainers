...
.DESCRIPTION
    ...
    BT1Trainer.exe. The app's manifest requests administrator rights (needed to
...
$tfm         = 'net8.0-windows'
$exePath     = Join-Path $root "src\BardsTale1Trainer\bin\$Configuration\$tfm\BT1Trainer.exe"
...
if ($Clean) {
    ...
    foreach ($dir in @(
            (Join-Path $root 'src\BardsTale1Trainer\bin'),
            (Join-Path $root 'src\BardsTale1Trainer\obj'))) {
