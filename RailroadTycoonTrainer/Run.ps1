...
<#
.SYNOPSIS
    Builds and launches the Railroad Tycoon trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\RailroadTycoonTrainer\RailroadTycoonTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'RRTTrainer.exe'
$exePath     = Join-Path $root "src\RailroadTycoonTrainer\bin\$Configuration\net8.0-windows\RRTTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Start Railroad Tycoon in DOSBox, then in the Live tab pick the dosbox process, Attach, and click "Auto-locate cash".'
# ------------------------------------------------------------------------------
