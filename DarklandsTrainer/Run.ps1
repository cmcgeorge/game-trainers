<#
.SYNOPSIS
    Builds and launches the Darklands trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\DarklandsTrainer\DarklandsTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'DarklandsTrainer.exe'
$exePath     = Join-Path $root "src\DarklandsTrainer\bin\$Configuration\net8.0-windows\DarklandsTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Darklands in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
