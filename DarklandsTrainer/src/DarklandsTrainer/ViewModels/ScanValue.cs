...
/// <summary>Parsing/formatting helpers for the numeric values the scanner reads and writes.</summary>
public static class ScanValue
{
    /// <summary>
    /// Parses user-typed text as a signed value, accepting either decimal (<c>39</c>) or hex
    /// (<c>0x27</c> / <c>27h</c>). Returns false on empty or malformed input.
    /// </summary>
    ...
    /// <summary>
    /// Folds a signed value into the unsigned representation the scanner actually stores, so a typed
    /// <c>-1</c> matches the <c>0xFF</c> / <c>0xFFFF</c> / <c>0xFFFFFFFF</c> the searcher decodes. Values
    /// already in the unsigned range pass through unchanged.
    /// </summary>
