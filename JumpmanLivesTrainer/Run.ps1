<#
.SYNOPSIS
    Builds and launches the Jumpman Lives! trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\JumpmanLivesTrainer\JumpmanLivesTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'JMLTrainer.exe'
$exePath     = Join-Path $root "src\JumpmanLivesTrainer\bin\$Configuration\net8.0-windows\JMLTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Start Jumpman Lives! in DOSBox, then in the Live tab pick the dosbox process, Attach, and the game is located automatically.'
# ------------------------------------------------------------------------------
