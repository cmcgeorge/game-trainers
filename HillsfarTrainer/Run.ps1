...
<#
.SYNOPSIS
    Builds and launches the Hillsfar trainer.
...
$root         = $PSScriptRoot
$project      = Join-Path $root 'src\HillsfarTrainer\HillsfarTrainer.csproj'
$testProject  = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName      = 'HillsfarTrainer.exe'
$exePath      = Join-Path $root "src\HillsfarTrainer\bin\$Configuration\net8.0-windows\HillsfarTrainer.exe"
$binConfigDir = 'bin'
$buildArgs    = @()
$launchHint   = 'Launch Hillsfar in DOSBox, load a character at the camp menu, then Attach in the trainer.'
# ------------------------------------------------------------------------------
