...
<#
.SYNOPSIS
    Builds and launches the Legacy of the Ancients trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\LegacyOfTheAncientsTrainer\LegacyOfTheAncientsTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'LoATrainer.exe'
$exePath     = Join-Path $root "src\LegacyOfTheAncientsTrainer\bin\$Configuration\net8.0-windows\LoATrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Legacy of the Ancients in DOSBox, then Attach in the trainer.'
