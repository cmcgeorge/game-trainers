...
<#
.SYNOPSIS
    Builds and launches the Mines of Titan trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\MinesOfTitanTrainer\MinesOfTitanTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'MoTTrainer.exe'
$exePath     = Join-Path $root "src\MinesOfTitanTrainer\bin\$Configuration\net8.0-windows\MoTTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Mines of Titan in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
