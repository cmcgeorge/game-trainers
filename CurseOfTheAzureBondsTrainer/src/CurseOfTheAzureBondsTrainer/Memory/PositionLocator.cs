...
public enum PositionEncoding
{
    ...
    /// <summary>
    /// Two adjacent little-endian 16-bit words, [X][Y], where the printed X is the stored word plus
    /// a constant bias. This is how the sister game holds the party's position on its overland map,
    /// and the shape is kept here because it costs nothing to look for and cannot survive a wrong
    /// guess: the bias is measured per candidate rather than assumed, and any candidate whose bias
    /// stops predicting the coordinates dies at the first <see cref="PositionLocator.Narrow"/>.
    /// Curse's own areas are all indoor 16×16 levels using <see cref="AdjacentBytes"/>, so this is
    /// a fallback, not the expected answer — see <c>docs/reverse-engineering.md</c> §7b.
    /// </summary>
    WildernessWords,
}
...
///
/// <para><b>Two shapes are searched for, not one.</b> The expected answer in Curse is three adjacent
/// bytes, <c>[X][Y][Facing]</c> — every area in the game is an indoor 16×16 level. The second shape,
/// a pair of biased 16-bit words, is how the sister game stores its overland position; scanning for
/// it as well costs one extra pattern per byte and cannot produce a wrong lock, because a candidate
/// has to keep predicting the coordinates through every <see cref="Narrow"/> to survive. Whichever
/// shape is right wins on the evidence rather than on an assumption about which map the party is on.
/// See <c>docs/reverse-engineering.md</c> §7b.</para>
///
/// <para>What coordinates alone cannot tell you is <i>which</i> 16×16 level you are on. That is
/// answered separately, and directly, by <see cref="MapLocator"/>.</para>
/// </summary>
