...
/// — HP (byte) at the record's <see cref="CoabFormat.OffHpMax"/>, measured relative to the
...
public static class CreationScanner
{
    ...
    public const int StatCount = CoabFormat.StatCount;
    ...
    // record layout so it can never drift from CoabFormat. The create buffer is a scratch character
    // record, so this holds there too when the record shape is confirmed.
    public const int SigToHp = CoabFormat.OffHpMax - CoabFormat.OffStr;   // 0x22
