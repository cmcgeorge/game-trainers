...
/// <summary>
/// Scans a target process for Curse of the Azure Bonds character/monster records by testing the
...
public static class CharacterLocator
{
        ...
        IProgress<double>? progress = null, CancellationToken ct = default)
    {
        ...
        byte[] buf = new byte[ChunkSize + CoabFormat.RecordSize];
        ...
        foreach (var region in regions)
        {
            ...
            for (nuint offset = 0; offset < region.Size;)
            {
                ...
                int readWant = (int)Math.Min((nuint)(ChunkSize + CoabFormat.RecordSize), region.Size - offset);
                int read = mem.Read(region.Base + offset, buf, readWant);
                if (read < CoabFormat.RecordSize)
                {
                    scanned += (nuint)want;
                    break;
                }

                for (int i = 0; i + CoabFormat.RecordSize <= read; i++)
                {
                ...
                nuint advance = read >= want
                    ? (nuint)want
                    : (nuint)Math.Max(1, read - CoabFormat.RecordSize + 1);
                offset += advance;
                scanned += advance;
    ...
    public const int SweepBufferSize = SweepChunk + CoabFormat.RecordSize;
    ...
    public static List<LocatedCharacter> FindCombatants(ProcessMemory mem, nuint partyLow, nuint partyHigh, byte[] buffer)
    {
        ...
        for (nuint addr = start; addr < end;)
        {
            ...
            // Read an extra record's worth so a record straddling a chunk edge is still seen.
            int read = mem.Read(addr, buffer, Math.Min(want + CoabFormat.RecordSize, buffer.Length));
            ...
            addr += read >= want ? (nuint)want
                  : read > CoabFormat.RecordSize ? (nuint)(read - CoabFormat.RecordSize + 1)
                  : (nuint)want;
        }
    ...
    {
        if (mem.Read(address, buffer, CoabFormat.RecordSize) != CoabFormat.RecordSize) return false;
