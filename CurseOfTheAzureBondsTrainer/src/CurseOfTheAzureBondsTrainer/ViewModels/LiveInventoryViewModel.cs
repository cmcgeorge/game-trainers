...
public sealed class LiveInventoryCharacterViewModel : ObservableObject
{
    ...
    /// <summary>
    /// Re-reads this character's 422-byte record from the game before its item list is walked.
    ///
    /// <para>Without this the walk starts from whatever the item-list head pointer
    /// (<see cref="CoabFormat.OffItemsPtr"/>) held at scan time, and the game rewrites that pointer
    ...
    public bool RefreshRecord(ProcessMemory mem, byte[] scratch)
    {
        ...
        Array.Copy(scratch, 0, Record.Bytes, 0, CoabFormat.RecordSize);
        return true;
    }
...
public sealed class LiveInventoryViewModel : ObservableObject, ILiveItemHost
{
    ...
    private readonly byte[] _recordBuf = new byte[CoabFormat.RecordSize];
