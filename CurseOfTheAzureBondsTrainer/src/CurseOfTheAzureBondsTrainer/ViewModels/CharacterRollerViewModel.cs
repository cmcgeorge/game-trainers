...
/// <summary>
/// "Roll a hero": automates the Curse of the Azure Bonds create-a-character re-roll. The roll lives in a
...
public sealed class CharacterRollerViewModel : ObservableObject
{
    ...
    public CharacterRollerViewModel(Func<ProcessMemory?> getMem, Func<int?> getPid, Action<string> setStatus)
    {
        ...
        {
            var stat = new RollStatViewModel(CoabFormat.StatsShort[i], CreationScanner.MaxStat,
    ...
    public string[] StatNames => CoabFormat.StatsShort;
    ...
    private async void Lock()
    {
        ...
        for (int i = 0; i < CoabFormat.StatCount; i++) captured[i] = Stats[i].Captured ?? 0;
        captured[CoabFormat.StatCount] = StrPercent.Captured ?? 0;
