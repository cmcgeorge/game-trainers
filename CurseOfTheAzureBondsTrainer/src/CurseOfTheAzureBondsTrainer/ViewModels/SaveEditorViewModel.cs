...
/// rewriting the .FX files. Edits files only — the game must be closed and the save reloaded.
/// </summary>
public sealed class SaveEditorViewModel : ObservableObject
{
    ...
    private const string DefaultSaveFolder = @"C:\Temp\Games\CURSE";
        ...
        try
        {
            if (!SaveGame.LooksLikeSaveFolder(SaveFolder))
            {
                Status = "No CHRDATn.SAV save files found in that folder. " +
                         ...
                         "cloud_saves\\CURSE folder beside the game, not CURSE itself).";
                return;
            }
    ...
    private bool ReadyToWrite()
    {
        ...
        if (!_warnedGameRunning && SaveFolderLocator.EmulatorRunning())
        {
            ...
            if (!Confirm("Curse of the Azure Bonds looks like it is still running.\n\n" +
