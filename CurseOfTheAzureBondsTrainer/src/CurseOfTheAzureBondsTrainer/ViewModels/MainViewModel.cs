...
public sealed class MainViewModel : ObservableObject, ICharacterHost, IDisposable
{
    ...
    public IReadOnlyList<SpellSlotRow> SpellSlots => ClassRaceBook.SpellSlots;
    ...
    private string _status = "Launch Curse of the Azure Bonds in DOSBox, then pick the process and Attach.";
    ...
    public MainViewModel()
    {
        ...
        // The Maps tab reads the game's own GEO*.DAX levels to identify which one is loaded, and
        // they live beside the save folder the editor already found — so follow that box rather
        // than asking for the same path twice.
        Maps.GameFolder = SaveFolderLocator.GameFolderFor(SaveEditor.SaveFolder);
        SaveEditor.PropertyChanged += (_, e) =>
        {
            if (e.PropertyName == nameof(SaveEditorViewModel.SaveFolder))
                Maps.GameFolder = SaveFolderLocator.GameFolderFor(SaveEditor.SaveFolder);
        };
    ...
    private readonly byte[] _pollBuf = new byte[CoabFormat.RecordSize];
    ...
    private void SweepEnemies()
    {
        ...
        // The Maps tab sweeps the same window when it identifies the loaded level, so hand it the
        // range rather than recomputing it there.
        Maps.SetPartyRange(low, high);
