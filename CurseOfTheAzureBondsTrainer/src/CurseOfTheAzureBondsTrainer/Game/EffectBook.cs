...
/// <summary>
/// The Gold Box effect/"power" dictionary: the type byte of a 9-byte effect record and what it
/// does. Effects are stored per-character in the save's CHRDATAn.FX file — see
/// <see cref="SaveGame"/>. Beneficial effects are flagged as such; unknown/unused slots are omitted.
///
/// <para>The table is shared across the Gold Box engine, and a real Curse party confirms it applies
/// here unchanged: both paladins in the sample save carry effect <c>0x08</c> — "protected from evil",
/// which is exactly the paladin's permanent aura — both dwarves carry <c>0x1A</c>, <c>0x2F</c> and
/// <c>0x61</c>, the three dwarven bonuses, and the elf carries <c>0x6B</c>, 90% resistance to sleep
/// and charm. Four races and classes, four correct answers, on records nobody wrote for this table.
/// <c>test/FormatCheck</c> asserts those four.</para>
/// </summary>
