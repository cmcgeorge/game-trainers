...
<#
.SYNOPSIS
    Builds and launches the Curse of the Azure Bonds trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\CurseOfTheAzureBondsTrainer\CurseOfTheAzureBondsTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'CoabTrainer.exe'
$exePath     = Join-Path $root "src\CurseOfTheAzureBondsTrainer\bin\$Configuration\net8.0-windows\CoabTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Curse of the Azure Bonds in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
