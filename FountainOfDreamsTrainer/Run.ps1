<#
.SYNOPSIS
    Builds and launches the Fountain of Dreams trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\FountainOfDreamsTrainer\FountainOfDreamsTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'FODTrainer.exe'
$exePath     = Join-Path $root "src\FountainOfDreamsTrainer\bin\$Configuration\net8.0-windows\FODTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Fountain of Dreams in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
