<#
.SYNOPSIS
    Builds and launches the The Perfect General II trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\ThePerfectGeneral2Trainer\ThePerfectGeneral2Trainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'PG2Trainer.exe'
$exePath     = Join-Path $root "src\ThePerfectGeneral2Trainer\bin\$Configuration\net8.0-windows\PG2Trainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch The Perfect General II in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
