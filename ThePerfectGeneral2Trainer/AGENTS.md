...
lives in a run-time heap at an address that changes every session, and — unlike the sibling trainers —
that heap has **no strong constant static signature** to anchor a locator to (the one confirmed dynamic
structure sits in far-pointer soup, and the unit-rules tables are strided struct records, not
contiguous arrays). So there is deliberately **no `GameLocator`**. The dependable primitive is a
Cheat-Engine-style **value scan** via `GameTrainers.Common.Memory.MemorySearcher`, driven from
`MainViewModel`. See `.docs/ReverseEngineering.md` for the full teardown, confidence tags, and the open
Ghidra leads.
...
- `src/ThePerfectGeneral2Trainer/` — the WPF app (`AssemblyName` `PG2Trainer`, `RootNamespace`
  ...
  - `ViewModels/` — hand-rolled MVVM using `GameTrainers.Common.Mvvm` (`ObservableObject` exposes
    ...
    - `MainViewModel` — attach/detach, background scan `Task` with cancellation, 200 ms poll loop
      (re-writes frozen pins, live-refreshes a small result set), pin/freeze actions, the Buy Points
      guide. Implements `IScanHost` and `IDisposable`.
...
rejected), the `UNITINFO.DOC` unit reference, the value-parsing/width-fit helpers, and the
frozen-value view-model logic (poke, freeze re-write, out-of-width rejection, write-failure report). It
runs individual `Check(...)` assertions and returns exit code 0 (pass) or 1 (fail). Any parser/format
or view-model change must keep the assertions green.
...
separately) — pin it with the guided scan instead. The trainer is intentionally scoped to the value
scanner plus the two Confirmed data structures; the placed-unit records and the unit-rules record
stride are known open leads (`.docs/ReverseEngineering.md` §4/§6), left for Ghidra work rather than
guessed.
