...
public sealed class MainViewModel : ObservableObject, ICharacterHost, IDisposable
{
    ...
    private string _status = "Launch Amberstar in DOSBox, then pick the process and Attach.";
    ...
    {
        get => _freezeSp;
        set { if (SetField(ref _freezeSp, value)) { foreach (var c in Party) c.FreezeSp = value; Status = value ? "SP frozen for the party." : "SP freeze OFF."; } }
    }
    ...
    {
        get => _freezeStatus;
        set { if (SetField(ref _freezeStatus, value)) { foreach (var c in Party) c.FreezeStatus = value; Status = value ? "Status frozen for the party (no ailments)." : "Status freeze OFF."; } }
    }
    ...
    private void Detach()
    {
        ...
        _freezeSp = false; OnPropertyChanged(nameof(FreezeSp));
    ...
    private async void Scan()
    {
        ...
        IsScanning = true;
        Status = "Scanning memory for the Amberstar party…";
        _scanCts?.Dispose();
        ...
        try
        {
            ...
            if (FreezeSp) foreach (var c in Party) c.FreezeSp = true;
