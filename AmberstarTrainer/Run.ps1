...
<#
.SYNOPSIS
    Builds and launches the Amberstar trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\AmberstarTrainer\AmberstarTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'AmberstarTrainer.exe'
$exePath     = Join-Path $root "src\AmberstarTrainer\bin\$Configuration\net8.0-windows\AmberstarTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Amberstar in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
