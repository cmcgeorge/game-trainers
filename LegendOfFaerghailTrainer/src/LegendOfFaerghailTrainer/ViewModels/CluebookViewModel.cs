...
using LegendOfFaerghailTrainer.Cluebooks;
...
public sealed class CluebookViewModel : ObservableObject
{
    ...
    private string _status = "Choose the sections to include, then save the self-contained cluebook.";
    ...
    private void Save()
    {
        ...
        {
            IncludeMaps = IncludeMaps, IncludeSpells = IncludeSpells, IncludeItems = IncludeItems,
            IncludeClasses = IncludeClasses, IncludeWalkthrough = IncludeWalkthrough, IncludeStrategy = IncludeStrategy
        });
        string path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Legend-of-Faerghail-cluebook.html");
