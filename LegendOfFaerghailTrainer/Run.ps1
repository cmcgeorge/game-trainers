...
<#
.SYNOPSIS
    Builds and launches the Legend of Faerghail trainer.
...
.EXAMPLE
    dotnet run --project test\FormatCheck -- --live 1234 --game "C:\GAMES\LOF"
    Runs the harness against a running DOSBox and a copy of the game, which adds an
    end-to-end locate and parses the shipped ROST\ROST and GAMES\GAMEn files.
#>
[CmdletBinding()]
param(
...
$root         = $PSScriptRoot
$project      = Join-Path $root 'src\LegendOfFaerghailTrainer\LegendOfFaerghailTrainer.csproj'
$testProject  = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName      = 'LoFTrainer.exe'
$exePath      = Join-Path $root "src\LegendOfFaerghailTrainer\bin\$Configuration\net8.0-windows\LoFTrainer.exe"
$binConfigDir = 'bin'
$buildArgs    = @()
$launchHint   = 'Start the game with START.BAT in DOSBox, recruit at least one companion in the tavern, then Attach.'
# ------------------------------------------------------------------------------
...
Write-Host "Tip: the game has no speed limiter. Use the trainer's Slower button, or set 'cycles=fixed 3000' in dosbox.conf." -ForegroundColor DarkGray
