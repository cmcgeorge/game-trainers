...
/// <summary>
/// A running tally of the create-screen rolls read this lock session: per-stat and total-sum averages
/// and extremes. Unlike the Might &amp; Magic roller, Wasteland's create roll uses an undocumented
/// distribution (not plain 3d6), so this deliberately keeps only the neutral averages/extremes and
/// makes no fairness judgement. Pure logic with no UI dependency — the view model owns one instance,
/// feeds it each fresh roll on the roll-loop thread, and posts <see cref="Snapshot"/>s to the UI. Not
/// thread-safe; all access is expected on a single owner thread at a time.
/// </summary>
