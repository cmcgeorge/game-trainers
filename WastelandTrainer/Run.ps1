...
<#
.SYNOPSIS
    Builds and launches the Wasteland trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\WastelandTrainer\WastelandTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'WLTrainer.exe'
$exePath     = Join-Path $root "src\WastelandTrainer\bin\$Configuration\net8.0-windows\WLTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Wasteland in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
