<#
.SYNOPSIS
    Builds and launches the Wizardry 1 trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\Wizardry1Trainer\Wizardry1Trainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'Wiz1Trainer.exe'
$exePath     = Join-Path $root "src\Wizardry1Trainer\bin\$Configuration\net8.0-windows\Wiz1Trainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Wizardry 1 in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
