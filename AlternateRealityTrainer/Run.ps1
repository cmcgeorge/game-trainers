...
<#
.SYNOPSIS
    Builds and launches the Alternate Reality: The City trainer.
...
$root         = $PSScriptRoot
$project      = Join-Path $root 'src\AlternateRealityTrainer\AlternateRealityTrainer.csproj'
$testProject  = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName      = 'ARTrainer.exe'
$exePath      = Join-Path $root "src\AlternateRealityTrainer\bin\$Configuration\net8.0-windows\ARTrainer.exe"
$binConfigDir = 'bin'
$buildArgs    = @()
$launchHint   = 'Launch CITY.EXE in DOSBox and resume a character, then Attach in the trainer.'
# ------------------------------------------------------------------------------
