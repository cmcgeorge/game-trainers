...
/// re-written every poll tick so the game can't move it back. Freezing matters in Imperialism II
/// because the economic engine recomputes the treasury and warehouse totals each turn, so a one-shot
/// poke is undone at the next turn tick unless it is held. Writes follow the read-validate-write
