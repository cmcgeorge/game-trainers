...
<#
.SYNOPSIS
    Builds and launches the Wasteland Remastered trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\WastelandRemasteredTrainer\WastelandRemasteredTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'WLRTrainer.exe'
$exePath     = Join-Path $root "src\WastelandRemasteredTrainer\bin\$Configuration\net8.0-windows\WLRTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Wasteland Remastered, load or start a party, then Attach and Locate in the trainer.'
# ------------------------------------------------------------------------------
