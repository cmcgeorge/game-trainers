<#
.SYNOPSIS
    Builds and launches the BattleTech: The Crescent Hawk's Inception trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\BattleTech1Trainer\BattleTech1Trainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'BattleTechTrainer.exe'
$exePath     = Join-Path $root "src\BattleTech1Trainer\bin\$Configuration\net8.0-windows\BattleTechTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch BattleTech in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
