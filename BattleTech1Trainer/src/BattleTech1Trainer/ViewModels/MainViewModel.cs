...
using BattleTech1Trainer.Memory;
...
/// <summary>
/// Root view-model. BattleTech: The Crescent Hawk's Inception is a real-mode DOS game whose mutable
/// state (C-Bills, character health/armor, skill ordinals) lives in guest RAM at an address that
/// changes every DOSBox session (see <c>.docs/ReverseEngineering.md</c> §5). The reliable primitive is
/// therefore a Cheat-Engine-style <b>value scan</b>: attach to the emulator, snapshot memory, and
/// narrow by what the on-screen number does. Survivors are pinned to a freeze table that re-writes them
/// every poll tick. A <b>Detect game</b> button confirms the right process by signature-scanning for the
/// EXE's static strings, and read-only reference tabs surface the Confirmed 'Mech / weapon / skill data.
/// </summary>
public sealed class MainViewModel : ObservableObject, IScanHost, IDisposable
{
    ...
    public ObservableCollection<MechInfo> Mechs { get; } = new(MechReference.Mechs);
    public ObservableCollection<WeaponInfo> Weapons { get; } = new(WeaponReference.All);
    public ObservableCollection<SkillInfo> Skills { get; } = new(SkillSheet.Skills);
    ...
    private string _status = "Launch BattleTech in DOSBox, pick the emulator process, and Attach.";
    ...
    /// <summary>Signature-scans the attached process to confirm BattleTech is present.</summary>
    ...
    /// <summary>Sets up and explains the guided scan for the C-Bills total.</summary>
    public ICommand CBillsGuideCommand { get; }
    /// <summary>Sets up and explains the guided scan for a character's Health / Armor byte.</summary>
    public ICommand HealthArmorGuideCommand { get; }
    /// <summary>Sets up and explains the guided scan for a skill ordinal.</summary>
    ...
    public MainViewModel()
    {
        ...
        DetectGameCommand = new RelayCommand(_ => DetectGame(), _ => IsAttached && !IsScanning);
        ...
        CBillsGuideCommand = new RelayCommand(_ => ShowCBillsGuide(), _ => IsAttached && !IsScanning);
        HealthArmorGuideCommand = new RelayCommand(_ => ShowHealthArmorGuide(), _ => IsAttached && !IsScanning);
    ...
    private void Attach()
    {
        ...
        try
        {
            ...
            Status = $"Attached to {SelectedProcess.Name} (pid {SelectedProcess.Id}). " +
                     "Click Detect game to confirm it, or use a guided scan below.";
        }
        catch (Exception ex)
        {
    ...
    private async void DetectGame()
    {
        ...
        IsScanning = true;
        Status = "Scanning process memory for BattleTech signatures…";
        _scanCts?.Dispose();
        ...
        try
        {
            var result = await Task.Run(() => GameDetector.Detect(mem, ct), ct);
            if (mem != _mem || ct.IsCancellationRequested) return;
            Status = result.Found
                ? $"Detected BattleTech: matched \"{result.Signature}\" at 0x{(ulong)result.Address:X}. " +
                  "Right process — now use a guided scan to pin a value."
                : "No BattleTech signature found. Make sure the game is loaded past the title screen and " +
                  "that you attached to the emulator process (not a launcher).";
        }
        catch (OperationCanceledException) { Status = "Detection cancelled."; }
        catch (Exception ex) { Status = "Detection error: " + ex.Message; }
        finally
        {
            IsScanning = false;
            RaiseCommands();
    ...
    {
        BeginGuide(ScanWidth.Int32, "C-Bills");
        Status = "C-Bills guide: open a screen showing your C-Bill total, type it → First Scan; spend or " +
                 "earn some so the number changes → type the new total → Exact. Repeat until one row " +
                 "remains, then Pin it and edit the Target. (Try Int16 if Int32 finds nothing.)";
    }

    private void ShowHealthArmorGuide()
    {
        BeginGuide(ScanWidth.Byte, "Health/Armor");
        Status = "Health / Armor guide: open Inspect Character to read the value, type it → First Scan; " +
                 "take a hit or heal so it changes → type the new value → Exact (or use Decreased / " +
                 "Increased). Repeat until one row remains, then Pin and freeze it.";
    }

    private void ShowSkillGuide()
    {
        BeginGuide(ScanWidth.Byte, "Skill");
        Status = "Skill guide: skills are stored as a 0–4 ordinal (Unskilled…Excellent). Read the level " +
                 "on Inspect Character, type its number (Good = 3) → First Scan; train it up one → type the " +
                 "new number → Exact. Pin the survivor and set the Target to 4 for Excellent.";
    }
    ...
    private void RaiseCommands()
    {
        ...
        (DetectGameCommand as RelayCommand)?.RaiseCanExecuteChanged();
        ...
        (CBillsGuideCommand as RelayCommand)?.RaiseCanExecuteChanged();
        (HealthArmorGuideCommand as RelayCommand)?.RaiseCanExecuteChanged();
