...
<#
.SYNOPSIS
    Builds and launches the Questron II trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\Questron2Trainer\Questron2Trainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'Q2Trainer.exe'
$exePath     = Join-Path $root "src\Questron2Trainer\bin\$Configuration\net8.0-windows\Q2Trainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Questron II in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
