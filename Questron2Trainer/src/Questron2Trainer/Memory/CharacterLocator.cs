...
namespace Questron2Trainer.Memory;
...
/// <summary>
/// Locates the Questron II character record inside the attached emulator's memory.
///
/// Two strategies, tried in order:
///
/// 1. <b>Anchor</b> — the copyright string "Questron II (C) 1988 S.S.I." appears in the game's
///    data segment and loads verbatim into guest RAM. The locator scans for this string, then
///    searches a 256 KB window forward for a valid character record (see
///    <see cref="CharacterRecord.IsValidRecord"/>). The delta from the anchor to the character
///    is not fixed (it depends on the session's memory layout), so the window is scanned rather
///    than reading at a single hard-coded offset.
///
/// 2. <b>Structural</b> — a fallback used when the anchor is not present. Scans all readable
///    memory for a <see cref="CharacterFormat.RecordSize"/>-byte window that passes
///    <see cref="CharacterRecord.IsValidRecord"/> — a printable ASCII name starting with a
///    letter, plausible HP/Food/Gold, five attributes in 1..25, and a level in 0..20.
///
/// Questron II is a single-character RPG, so the locator returns at most one character.
/// </summary>
public static class CharacterLocator
{
    ...
    private static readonly byte[] AnchorBytes = Encoding.ASCII.GetBytes(GameFacts.CopyrightString);

    /// <summary>
    /// Finds the live character, or null if none can be located (not attached to Questron II,
    /// or the game isn't loaded past the title screen yet). Tries the anchor first, then the
    /// structural scan.
    /// </summary>
    ...
    // --- strategy 1: anchor + forward window scan ---------------------------
    private static LocatedCharacter? FindByAnchor(IMemorySource mem, CancellationToken ct)
    {
        ...
        {
            // Search a 256 KB window forward from the anchor for a valid character record.
