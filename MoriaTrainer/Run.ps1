<#
.SYNOPSIS
    Builds and launches the UMoria 5.5.2 trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\MoriaTrainer\MoriaTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'MoriaTrainer.exe'
$exePath     = Join-Path $root "src\MoriaTrainer\bin\$Configuration\net8.0-windows\MoriaTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch UMoria 5.5.2 in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
