...
<#
.SYNOPSIS
    Builds and launches the Roadwar 2000 trainer.
...
.EXAMPLE
    ...
    Builds Debug, runs the 703 format checks, and does not launch the GUI.

.EXAMPLE
    dotnet run --project test\FormatCheck -- --live
    Runs the checks and additionally attaches to a running DOSBox to verify the
    locator end to end. Skipped rather than failed when no emulator is running.
#>
[CmdletBinding()]
param(
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\Roadwar2000Trainer\Roadwar2000Trainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'Roadwar2000Trainer.exe'
$exePath     = Join-Path $root "src\Roadwar2000Trainer\bin\$Configuration\net8.0-windows\Roadwar2000Trainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Start START.EXE in DOSBox, then press Attach in the trainer.'
# ------------------------------------------------------------------------------
