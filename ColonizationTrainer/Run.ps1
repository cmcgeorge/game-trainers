...
<#
.SYNOPSIS
    Builds and launches the Colonization trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\ColonizationTrainer\ColonizationTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'ColTrainer.exe'
$exePath     = Join-Path $root "src\ColonizationTrainer\bin\$Configuration\net8.0-windows\ColTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Open a COLONYxx.SAV in the Save Editor, or launch Colonization in DOSBox and use the Live tab.'
# ------------------------------------------------------------------------------
