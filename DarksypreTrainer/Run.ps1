<#
.SYNOPSIS
    Builds and launches the DarkSpyre trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\DarksypreTrainer\DarksypreTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'DarksypreTrainer.exe'
$exePath     = Join-Path $root "src\DarksypreTrainer\bin\$Configuration\net8.0-windows\DarksypreTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch DarkSpyre in DOSBox, then Attach in the trainer.'
...
if (-not (Test-Path -LiteralPath $project)) {
    throw "Project not found at '$project'. Run this script from the DarksypreTrainer folder."
}
