<#
.SYNOPSIS
    Builds the Dragon Wars trainer and launches it.

.DESCRIPTION
    Restores/builds the WPF (.NET 8) project with `dotnet build`, then starts the produced
    DWTrainer.exe. The app's manifest requests administrator rights (needed to read/write
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\DragonWarsTrainer\DragonWarsTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$tfm         = 'net8.0-windows'
$exePath     = Join-Path $root "src\DragonWarsTrainer\bin\$Configuration\$tfm\DWTrainer.exe"
...
if ($Clean) {
    ...
    foreach ($dir in @(
            (Join-Path $root 'src\DragonWarsTrainer\bin'),
            (Join-Path $root 'src\DragonWarsTrainer\obj'),
...
Write-Step "Started. (Launch Dragon Wars in DOSBox, then Attach in the trainer.)"
