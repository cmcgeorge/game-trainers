...
<#
.SYNOPSIS
    Builds and launches the Dragon Wars trainer.
...
$exeName     = 'DWTrainer.exe'
$exePath     = Join-Path $root "src\DragonWarsTrainer\bin\$Configuration\net8.0-windows\DWTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Dragon Wars in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
