...
<#
.SYNOPSIS
    Builds and launches the Red Baron trainer.
...
.EXAMPLE
    ...
    Runs the harness against a copy of the game and a running DOSBox, which adds an
    end-to-end locate, the game-folder discovery and the emulator config checks.
    Read-only; safe to point at a game mid-mission.
#>
[CmdletBinding()]
param(
...
$root         = $PSScriptRoot
$project      = Join-Path $root 'src\RedBaronTrainer\RedBaronTrainer.csproj'
$testProject  = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName      = 'RedBaronTrainer.exe'
$exePath      = Join-Path $root "src\RedBaronTrainer\bin\$Configuration\net8.0-windows\RedBaronTrainer.exe"
$binConfigDir = 'bin'
$buildArgs    = @()
$launchHint   = 'Start the game with BARON.COM in DOSBox and let the main menu appear; the trainer attaches on its own.'
# ------------------------------------------------------------------------------
...
Write-Host "Tip: Red Baron times the game port itself, so set 'cycles=fixed 12000' and" -ForegroundColor DarkGray
Write-Host "     'joysticktype=2axis' in dosbox.conf - see the trainer's Joystick tab." -ForegroundColor DarkGray
