<#
.SYNOPSIS
    Builds and launches the BeachHead 2000 trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\BeachHead2000Trainer\BeachHead2000Trainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'BeachHead2000Trainer.exe'
$exePath     = Join-Path $root "src\BeachHead2000Trainer\bin\$Configuration\net8.0-windows\BeachHead2000Trainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch BeachHead 2000, then Attach in the trainer.'
# ------------------------------------------------------------------------------
