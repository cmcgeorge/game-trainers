...
<#
.SYNOPSIS
    Builds and launches the Lords of the Realm trainer.
...
.EXAMPLE
    .\Run.ps1 -NoBuild
    Launches the most recent build without rebuilding.
#>
[CmdletBinding()]
param(
...
$testProject = $null
$exeName     = 'LordsOfTheRealmTrainer.exe'
$exePath     = Join-Path $root "Trainer\bin\$Configuration\net9.0-windows\LordsOfTheRealmTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'In the app: press "Attach to DOSBox-X" with the game running.'
# ------------------------------------------------------------------------------
