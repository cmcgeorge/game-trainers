...
<#
.SYNOPSIS
    Builds and launches the Dark Designs I trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\DarkDesigns1Trainer\DarkDesigns1Trainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'DD1Trainer.exe'
$exePath     = Join-Path $root "src\DarkDesigns1Trainer\bin\$Configuration\net8.0-windows\DD1Trainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch Dark Designs I in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
