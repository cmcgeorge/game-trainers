<#
.SYNOPSIS
    Builds and launches the Civilization III: Conquests trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\Civilization3ConquestsTrainer\Civilization3ConquestsTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'Civ3ConqTrainer.exe'
$exePath     = Join-Path $root "src\Civilization3ConquestsTrainer\bin\$Configuration\net8.0-windows\Civ3ConqTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Start Civ3Conquests.exe and load or begin a game, then Attach in the trainer - it auto-locates with no value searching.'
# ------------------------------------------------------------------------------
