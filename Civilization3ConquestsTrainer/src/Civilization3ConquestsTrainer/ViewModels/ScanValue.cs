...
public static class ScanValue
{
    ...
    /// matches the <c>0xFF</c>/<c>0xFFFF</c>/<c>0xFFFFFFFF</c> the searcher decodes. Civ3 keeps several
    /// values that legitimately go negative — an unset race id is <c>-1</c>, and the encoded half of the
    /// treasury is routinely negative — so folding is not academic here.
    /// </summary>
