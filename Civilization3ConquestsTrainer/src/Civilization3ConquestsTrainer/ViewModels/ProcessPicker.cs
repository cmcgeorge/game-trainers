...
/// trainer's own executable is <c>Civ3ConqTrainer.exe</c>, whose process name contains "civ3" and
/// sorts <i>before</i> <c>Civ3Conquests</c> under an ordinal comparison — so a picker that merely
/// substring-matched and then sorted by name would auto-select the trainer itself, attach to a
/// 64-bit .NET process, and report that the target is not a 32-bit image. Exact matches therefore
/// outrank hints, and the trainer's own process is excluded outright.
/// </summary>
public static class ProcessPicker
{
    ...
    /// <summary>
    /// Picks the default selection: whatever was selected before if it is still there, else the best
    /// match, else nothing. Deliberately refuses to default to a hint-only match when no exact match
    /// exists — the user should have to choose, rather than be silently pointed at the wrong process.
    /// </summary>
