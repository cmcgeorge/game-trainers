...
/// re-written every poll tick so the game can't move it back. Freezing matters in Civ3 because the
/// turn boundary recomputes the economy and resets unit movement and damage, so a one-shot poke lasts
/// only until you press Enter. Writes follow the read-validate-write pattern via the host; a value
...
public sealed class FrozenValueViewModel : ObservableObject
{
    ...
    /// <summary>Optional human label for the pin (e.g. "Treasury"), shown in the freeze grid.</summary>
