...
public sealed class MemorySearchViewModel : ObservableObject
{
    ...
    public MemorySearchViewModel(Func<ProcessMemory?> getMem, Action<string> setStatus)
    {
        ...
        FirstScanExactCommand = new RelayCommand(() => RunScan((s, c) => s.FirstScanExact(ParseValue(ValueText), c)),
            () => CanScan && TryParseValue(ValueText, out _));
        FirstScanUnknownCommand = new RelayCommand(() => RunScan((s, c) => s.FirstScanUnknown(c)), () => CanScan);
        ...
        RefreshValuesCommand = new RelayCommand(() => RunScan((s, c) => s.RefreshValues(c)), () => HasResults);
    ...
    private void Narrow(ScanCompare compare)
    {
        ...
        RunScan((s, c) => s.NextScan(compare, target, c));
    }
    ...
    private void WriteSelected()
    {
        ...
        if (!FitsWidth(v, _searcher.Width))
        {
            _setStatus($"{v} doesn't fit a {_searcher.Width} ({WidthRangeText(_searcher.Width)}) — not written.");
            return;
        }
    ...
    private void Poke()
    {
        ...
        if (!FitsWidth(v, _width))
        {
            _setStatus($"{v} doesn't fit a {_width} ({WidthRangeText(_width)}) — not poked.");
            return;
        }
    ...
    // A value fits a width if it's representable in that many bytes as either an unsigned
    // (0..2^n-1) or signed (-2^(n-1)..2^(n-1)-1) integer. Without this, WriteValue silently
    // keeps only the low bytes while the UI reports the full number the user typed.
    private static bool FitsWidth(long v, ScanWidth width)
    {
        int bits = (int)width * 8;
        long signedMin = -(1L << (bits - 1));
        long unsignedMax = (1L << bits) - 1;
        return v >= signedMin && v <= unsignedMax;
    }

    private static string WidthRangeText(ScanWidth width)
    {
        int bits = (int)width * 8;
        long signedMin = -(1L << (bits - 1));
        long unsignedMax = (1L << bits) - 1;
        return $"{signedMin}..{unsignedMax}";
    }
