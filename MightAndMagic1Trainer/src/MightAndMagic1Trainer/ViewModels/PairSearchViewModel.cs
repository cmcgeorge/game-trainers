...
public sealed class PairSearchViewModel : ObservableObject
{
    ...
    private bool PatternValid =>
        InputParsing.TryParseByte(_xText, 0xFF, out _) && InputParsing.TryParseByte(_yText, 0xFF, out _);
    ...
    {
        if (!InputParsing.TryParseByte(_xText, 0xFF, out int x) || !InputParsing.TryParseByte(_yText, 0xFF, out int y))
            return null;
