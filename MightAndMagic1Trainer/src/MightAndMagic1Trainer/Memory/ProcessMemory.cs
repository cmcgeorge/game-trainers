...
using static MightAndMagic1Trainer.Memory.NativeMethods;
    ...
    public int Read(nuint address, byte[] buffer, int count)
    {
        if ((uint)count > (uint)buffer.Length)
            throw new ArgumentOutOfRangeException(nameof(count),
                "Requested read size exceeds the destination buffer — would corrupt the managed heap.");
        try
        {
    ...
    public bool WriteRange(nuint address, byte[] buffer, int offset, int length)
    {
        if (offset < 0 || length < 0 || offset + length > buffer.Length)
            throw new ArgumentOutOfRangeException(nameof(length),
                "WriteRange slice falls outside the source buffer.");
