...
public sealed class Cluebook
{
                                           ...
                                           IReadOnlyList<LocationChapter> chapters)
    {
        ...
        notes.Add($"The numbered marks on the plans are coordinates published in walkthroughs, not squares this " +
                  $"project decoded -- which is why there are {LandmarkBook.Landmarks.Count} of them across " +
                  $"{marked} places rather than one on every map. Each location's overlay does hold a table of " +
                  "the squares its events fire on, but what those numbers index has not been worked out, so the " +
                  "book can tell you a place has fourteen event squares and not which fourteen. The walls you " +
                  "can walk through are the opposite case: those are computed from the maze data and are exact.");
