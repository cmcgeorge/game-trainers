<#
.SYNOPSIS
    Builds and launches the War of the Lance trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\WarOfTheLanceTrainer\WarOfTheLanceTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'WOTLTrainer.exe'
$exePath     = Join-Path $root "src\WarOfTheLanceTrainer\bin\$Configuration\net8.0-windows\WOTLTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch War of the Lance in DOSBox, then Attach in the trainer.'
# ------------------------------------------------------------------------------
