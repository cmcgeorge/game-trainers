...
<#
.SYNOPSIS
    Builds and launches the Bard's Tale Trilogy trainer.
...
$root        = $PSScriptRoot
$project     = Join-Path $root 'src\BardsTaleTrilogyTrainer\BardsTaleTrilogyTrainer.csproj'
$testProject = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName     = 'BTTTrainer.exe'
$exePath     = Join-Path $root "src\BardsTaleTrilogyTrainer\bin\$Configuration\net8.0-windows\BTTTrainer.exe"
$binConfigDir = 'bin'
$buildArgs   = @()
$launchHint  = 'Launch The Bard''s Tale Trilogy, load or start a party, then Attach and Locate in the trainer.'
# ------------------------------------------------------------------------------
