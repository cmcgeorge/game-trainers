...
<#
.SYNOPSIS
    Builds and launches the Airborne Ranger trainer.
...
.PARAMETER Test
    Run the FormatCheck verification harness after building.
...
.EXAMPLE
    ...
    Builds Debug, runs the format checks, and does not launch the GUI. The checks need
    neither the game nor an emulator.
#>
[CmdletBinding()]
param(
...
$root         = $PSScriptRoot
$project      = Join-Path $root 'src\AirborneRangerTrainer\AirborneRangerTrainer.csproj'
$testProject  = Join-Path $root 'test\FormatCheck\FormatCheck.csproj'
$exeName      = 'ARangerTrainer.exe'
$exePath      = Join-Path $root "src\AirborneRangerTrainer\bin\$Configuration\net8.0-windows\ARangerTrainer.exe"
$binConfigDir = 'bin'
$buildArgs    = @()
$launchHint   = 'Launch AR.EXE in DOSBox and get into a mission, then Attach in the trainer.'
# ------------------------------------------------------------------------------
