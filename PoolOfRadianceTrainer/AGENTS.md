...
The combat panel's Weaken/Kill also have standing forms (`MainViewModel.AutoWeaken`/`AutoKill`), applied from the poll tick to whatever the arena sweep is listing. Three rules hold there, and each is load-bearing rather than defensive. It must re-read and re-identify every record itself (`CharacterLocator.Reread`) instead of trusting the enemy list, which can be a tick stale and is never swept at all while the party is unlocated — an unattended write through a freed slot lands in whatever took it over, and the party shares the same 640 KiB DOS heap. It must skip a creature that is already out of the fight (`CharacterViewModel.IsOutOfTheFight`), since writing a hit point into a record the party has already dropped is how such a pass puts a corpse back on the field; note that check tests `HpCurrent > HpMax` as well as `<= 0`, because current HP is an unsigned byte and a creature taken below zero reads back wrapped. And it must be idempotent, since it runs against every enemy roughly twice a second.

Weakened records stay recognisable to the sweep only through `CharacterRecord.LooksWeakened`, which tests the armour-class pair and nothing else. Don't fold that exemption into the AC plausibility band (that band is what keeps look-alike buffers out), and don't "tighten" it by adding the other fields Weaken writes — HP and THAC0 are fields the game itself moves, so testing them reintroduces the drop-out the mark exists to prevent. `CharacterRecord.IsWeakened` is the strict all-five test, and it answers the different question of whether the auto pass still has work to do.
